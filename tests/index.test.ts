...
describe('transform utility', () => {
  test('px -> vw', () => {
    expect(transform('160px', defaultOptions)).toBe('42.66667vw')
  })

  test('rem -> vw', () => {
    expect(transform('10rem', defaultOptions)).toBe('42.66667vw')
  })

  test('edge cases', () => {
    expect(transform('0px', defaultOptions)).toBe('0vw')
    expect(transform('-10px', defaultOptions)).toBe('-2.66667vw')
    expect(transform('1.5px', defaultOptions)).toBe('0.4vw')
    expect(transform('0.5rem', defaultOptions)).toBe('2.13333vw')
    expect(transform('-2rem', defaultOptions)).toBe('-8.53333vw')
  })

  test('mixed values', () => {
    expect(transform('10px 20rem', defaultOptions)).toBe('2.66667vw 85.33333vw')
    expect(transform('margin: 10px 5rem 20px 2rem', defaultOptions)).toBe(
      'margin: 2.66667vw 21.33333vw 5.33333vw 8.53333vw'
    )
  })

  test('no conversion needed', () => {
    expect(transform('50vw', defaultOptions)).toBe('50vw')
    expect(transform('auto', defaultOptions)).toBe('auto')
    expect(transform('100%', defaultOptions)).toBe('100%')
    expect(transform('inherit', defaultOptions)).toBe('inherit')
  })

  test('complex values', () => {
    expect(transform('calc(100px + 2rem)', defaultOptions)).toBe('calc(26.66667vw + 8.53333vw)')
    expect(transform('rgba(255, 255, 255, 0.5) 10px', defaultOptions)).toBe('rgba(255, 255, 255, 0.5) 2.66667vw')
  })
})
...
describe('configuration options', () => {
  ...
  test('minPixelValue threshold for px', () => {
    ...
    expect(transform('3px', options)).toBe('0.8vw') // 大于阈值，转换
  })
...
describe('performance optimizations', () => {
  test('combined regex handles mixed units correctly', () => {
    expect(transform('10px 2rem 15px 1.5rem', defaultOptions)).toBe('2.66667vw 8.53333vw 4vw 6.4vw')
    expect(transform('border: 1px solid; padding: 10px 2rem', defaultOptions)).toBe(
      'border: 0.26667vw solid; padding: 2.66667vw 8.53333vw'
    )
  })

  test('performance with large values', () => {
    const largeValue = '10px 20px 30px 40px 50px 1rem 2rem 3rem 4rem 5rem'.repeat(10)
    const result = transform(largeValue, defaultOptions)
  ...
  test('single regex processes all units in one pass', () => {
    ...
    const result = transform(mixedValue, defaultOptions)
